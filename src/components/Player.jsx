...
export function Player({ track, onClose }) {
  ...
  const [played, setPlayed] = useState(0); // 0.0 to 1.0
  ...
  return (
    // COMPACT WIDTH: w-[320px] instead of 380px
    <div className="h-full flex flex-col relative w-full h-full text-xs">
      ...
      <div className="flex-1 p-4 space-y-3 overflow-y-auto bg-transparent">
        ...
        {error ? (
           <div className="p-2 bg-red-950/30 border border-red-500/30 text-red-200 text-[10px] text-center rounded">
              <span className="font-bold block">RESTRICTED</span>
              <a href={`https://www.youtube.com/watch?v=${youtubeId}`} target="_blank" rel="noreferrer" className="underline hover:text-white">Open External</a>
           </div>
        ) : (
